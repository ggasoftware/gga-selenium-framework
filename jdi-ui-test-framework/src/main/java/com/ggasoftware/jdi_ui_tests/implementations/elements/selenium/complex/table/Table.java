...
import com.ggasoftware.jdi_ui_tests.implementations.elements.selenium.base.ContextType;
...
public class Table<T extends SelectElement> extends Text implements ITable<T> {
    public Table() {
        this(null);
        //GetFooterFunc = t => t.FindElements(By.xpath("//tfoot/tr/td")).Select(el => el.AText).ToArray();
    }
    ...
    private T createCell(int colIndex, int rowIndex) {
        ...
        try {
            if (cellCreateTemplate != null) {
                ...
                cell.getLocationInfo().byLocator = fillByTemplateSilent(cell.getLocator(), rowIndex, colIndex);
                cell.getLocationInfo().context.add(Locator, getLocator());
            } else
                cell = (T) createCellInstance(clazz, WebDriverByUtils.fillByTemplate(_cellLocatorTemplate, rowIndex, colIndex));
    ...
    public <TChild extends SelectElement> TChild createCellInstance(Class<TChild> childClass, By newLocator) {
        ...
        element.getLocationInfo().byLocator = newLocator;
        element.getLocationInfo().context.add(ContextType.Locator, getLocator());
        return element;
    }
}